...
func main() {
	...
	tmplFile, err := ioutil.ReadFile("set.tmpl")
	if err != nil {
		log.Fatal(err)
	}
	tmpl, err := template.New("default").Parse(string(tmplFile))
	if err != nil {
		log.Fatal(err)
	}
	for _, template := range templates {
		...
		defer file.Close()
		if err := tmpl.Execute(file, template); err != nil {
			log.Fatal(err)
		}
	}
}
